          ...
          <AnimatePresence>
            {shortcutsVisible && (
              <motion.div 
                ...
                exit={{ opacity: 0, y: -20 }}
                className="absolute top-4 right-8 z-50 w-64 bg-white border border-slate-200 shadow-2xl rounded-2xl p-6"
              >
                <div className="flex items-center justify-between mb-4">
                  <h4 className="font-black text-xs uppercase tracking-widest text-primary">Shortcuts</h4>
                  <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => setShortcutsVisible(false)}><X size={14} /></Button>
                </div>
                <div className="space-y-3">
                  {[
                    { key: 'Space', desc: 'Reveal/Hide' },
                    { key: '→', desc: 'Next Question' },
                    { key: 'B', desc: 'Black Screen' },
                    { key: 'L', desc: 'Logo Screen' },
                    { key: 'T', desc: 'Toggle Timer' },
                    { key: '1-8', desc: 'Host Buzz' },
                    { key: 'S', desc: 'Scoreboard' },
                    { key: 'Esc', desc: 'Clear Buzz' }
                  ].map(s => (
                    <div key={s.key} className="flex justify-between items-center text-[10px]">
                      <span className="font-mono bg-slate-100 px-2 py-1 rounded border border-slate-200 font-bold">{s.key}</span>
                      <span className="font-bold text-slate-500 uppercase">{s.desc}</span>
                    </div>
                  ))}
                </div>
              </motion.div>
            ...
            {previewVisible && (
              <motion.div 
                ...
                animate={{ opacity: 1, x: 0 }}
                exit={{ opacity: 0, x: 20 }}
                className="absolute top-4 left-8 right-8 bottom-4 z-40 bg-slate-900/10 backdrop-blur-sm rounded-3xl border-2 border-dashed border-primary/30 p-8 flex flex-col"
              >
                 <div className="flex items-center justify-between mb-4">
                   <div className="flex items-center gap-2">
                     <Monitor className="text-primary" size={20} />
                     <h4 className="font-black text-xs uppercase tracking-widest text-primary">Projector Preview</h4>
                   </div>
                   <Button variant="destructive" size="sm" className="font-black text-[10px]" onClick={() => setPreviewVisible(false)}>CLOSE PREVIEW</Button>
                 </div>
                 <div className="flex-1 bg-white rounded-2xl shadow-inner overflow-hidden border border-slate-200 relative">
